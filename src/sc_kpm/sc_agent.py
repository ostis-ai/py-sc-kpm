...
class ScAgentClassic(ScAgent, ABC):
    def __init__(
        ...
        self._action_class = sc_keynodes.resolve(action_class_name, sc_types.NODE_CONST_CLASS)

    def __repr__(self) -> str:
        description = f"ClassicScAgent(action_class_name={repr(self._action_class_name)}"
        if self._event_element != sc_keynodes.get(QuestionStatus.QUESTION_INITIATED):
