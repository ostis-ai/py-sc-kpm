...
from sc_client.models import AsyncScEventParams, ScAddr, ScConstruction, ScTemplate
...
async def get_action_arguments(action_node: ScAddr, count: int) -> List[ScAddr]:
    ...
    for index in range(1, count + 1):
        argument = await get_element_by_role_relation(action_node, keynodes.rrel_index(index))
...
async def create_action_answer(action_node: ScAddr, *elements: ScAddr) -> None:
    ...
    await create_norole_relation(action_node, answer_struct_node, keynodes[CommonIdentifiers.NREL_ANSWER])
...
async def get_action_answer(action_node: ScAddr) -> ScAddr:
    ...
    if search_results := await client.template_search(templ):
...
async def execute_agent(
    ...
    result = await check_edge(sc_types.EDGE_ACCESS_VAR_POS_PERM, keynodes[reaction], question)
...
async def create_action(*concepts: Idtf) -> ScAddr:
    ...
    for concept in concepts:
        construction.create_edge(
            ...
            await keynodes.resolve(concept, sc_types.NODE_CONST_CLASS),
            ScAlias.ACTION_NODE,
        )
    action_node = await client.create_elements(construction)[0]
    return action_node


async def add_action_arguments(action_node: ScAddr, arguments: Dict[ScAddr, IsDynamic]) -> None:
    rrel_dynamic_arg = await keynodes.get_valid(CommonIdentifiers.RREL_DYNAMIC_ARGUMENT)
    index: int
    argument: ScAddr
    for index, (argument, is_dynamic) in enumerate(arguments.items(), 1):
        if argument.is_valid():
            rrel_i = keynodes.rrel_index(index)
            if is_dynamic:
...
async def execute_action(
    ...
    result = await check_edge(sc_types.EDGE_ACCESS_VAR_POS_PERM, keynodes[reaction], action_node)
    return result
...
async def wait_agent(seconds: float, question_node: ScAddr, reaction_node: ScAddr = None) -> None:
    reaction_node = reaction_node or keynodes[QuestionStatus.QUESTION_FINISHED]
    finish_event = Event()
    ...
    event_params = AsyncScEventParams(question_node, ScEventType.ADD_INGOING_EDGE, event_callback)
    sc_event = await client.events_create(event_params)[0]
    if not await check_edge(sc_types.EDGE_ACCESS_VAR_POS_PERM, reaction_node, question_node):
        finish_event.wait(seconds)
    await client.events_destroy(sc_event)
...
async def finish_action(action_node: ScAddr, status: Idtf = QuestionStatus.QUESTION_FINISHED) -> ScAddr:
    return await create_edge(sc_types.EDGE_ACCESS_CONST_POS_PERM, keynodes[status], action_node)
